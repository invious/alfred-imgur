on run
	...
	try
		do shell script "test -f /usr/local/bin/torsocks"
	on error
		return "Please make sure both torsocks is installed!"
	end try
	...
	try
		do shell script "/bin/ls ~/.alfred-imgur-tor.conf"
	on error errMsg number errNum
		return "Couldn't find Imgur API key (~/.alfred-imgur-tor.conf)."
	end try
...
on imgur(_Image, _isOpen)
	...
	set apikey to do shell script "/bin/cat ~/.alfred-imgur-tor.conf"
	
	set curlCommand to "torsocks curl -H \"Authorization: Client-ID " & apikey & "\" -F \"image=@" & sImg & "\" https://api.imgur.com/3/upload"
